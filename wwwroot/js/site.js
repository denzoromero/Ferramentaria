...
/*    const table = document.querySelector("#myTable"); // Select your table // by Id Table No advisable to use*/
    ...
    const paginationLink = document.querySelectorAll('.page-link');
    ...
    if (themeToggle.checked) {
     ...
     /*   if (table) table.classList.add('table-dark');*/
        ...
        paginationLink.forEach(link => {
            const parentItem = link.parentElement; // Get the parent .page-item element
            if (!parentItem.classList.contains('active')) { // Check if the parent does not have the 'active' class
                link.classList.remove('bg-light');
                link.classList.remove('text-black');
                link.classList.add('bg-dark');
                link.classList.add('text-white');
